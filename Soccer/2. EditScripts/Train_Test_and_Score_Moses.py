...
def return_bleu(runnumber, distortion, lm, wordpenalty, phrasepenalty, translationmodel1, translationmodel2, translationmodel3, translationmodel4,  unknownwordpenalty):
    ...
    os.system('/vol/customopt/machine-translation/bin/moses -f ' + currentpath + '/model/moses.ini -i /vol/tensusers/cvdlee/MosesFiles/Corpora/soccer/Test_gaps.data > ' + currentpath + '/PredictionFiles/Run' + str(runnumber) + '.txt')



    #Get the BLEU score
    warnings.filterwarnings(action='ignore', category=UserWarning)
    bleu = bleu_score('/vol/tensusers/cvdlee/MosesFiles/Corpora/soccer/Test_gaps.text', currentpath + '/PredictionFiles/Run' + str(runnumber) + '.txt')
    return bleu
...
def final_bleu():
    ...
    os.system('/vol/customopt/machine-translation/bin/moses -f ' + currentpath + '/model/moses.ini -i /vol/tensusers/cvdlee/MosesFiles/Corpora/soccer/Test_gaps.data > ' + currentpath + '/PredictionFiles/Final.txt')

    #Get the BLEU score
    warnings.filterwarnings(action='ignore', category=UserWarning)
    bleu = bleu_score('/vol/tensusers/cvdlee/MosesFiles/Corpora/soccer/Test_gaps.text', currentpath + '/PredictionFiles/Final.txt')
    return bleu